...
class PlayController:
    def __init__(self):
        ...
        #第一个问题：T时刻花多少（pacing)，通过参竟率来控制展现速度，实现匀速播放（匀速播放指的是 广告消耗曲线和大盘消耗曲线一致）
        #第一个问题：算法描述：当 广告的累计消耗 > 大盘消耗比 * 广告当日预算，播放速度加少，否则播放速度加快 （ 公式法LINKIN/PID算法/强化学习：action是广告下一步时刻的播放概率rate
